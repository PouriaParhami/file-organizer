...
class FileOrganizerGui:
    ...
    def define_widgets(self):
        ...
        self.label_distination_address = tk.Label(self.frame_left, text="distination Address")
        ...
        # # ListBox
        self.source_listbox = tk.Listbox(self.frame_right,)
            ...
            def wrapper(self):
                ...
                if roots == "b":
                    ...
                    if not self.distination_path.is_absolute():
                        self.label_message_label.configure(text="Check Your Destination Address!")
                    else:
                        result = f(self)
                        return result
                ...
                for category, extentions in self.FILE_CATEGORIES.items():
                    if item.suffix in extentions:
                        try:
                            ...
                            messagebox.showerror("Error", f"Your source address{source_path} and distination address {distination_path} are the same.")
                        except shutil.Error as e:
                            if "already exists" in str(e):
                                new_filename = item.name.rsplit('.', 1)[0] + "_1." + item.suffix
                                new_destination = distination_path / category / new_filename
                                shutil.copy(item, new_destination)
                                self.label_message_label.configure(text=f"Renamed file to {new_filename} and moved to {distination_path}")
                                print(f"Renamed file to {new_filename} and moved to {distination_path}")
                            else:
